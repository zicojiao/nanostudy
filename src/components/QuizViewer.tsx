...
function parseQuiz(text: string): Quiz | null {
  ...
  // Find question line (more flexible matching)
  const questionLineIndex = lines.findIndex((l) => l.startsWith("Q:") || l.match(/^Q[:\s]/));
  if (questionLineIndex === -1) {
    console.error("Quiz parse error: No question found. Text:", text);
    return null;
  }
  
  const question = lines[questionLineIndex].replace(/^Q[:.]?\s*/, "").trim();
  
  // Find options (A), B), C), D) or A. B. C. D.)
  const optionLines: string[] = [];
  let optionIndex = questionLineIndex + 1;
  const optionPattern = /^[A-D][)\-\.]\s*(.+)/i;
  
  while (optionIndex < lines.length && optionLines.length < 4) {
  ...
  if (optionLines.length < 4) {
    console.error("Quiz parse error: Not enough options found. Found:", optionLines.length, "Text:", text);
    return null;
  }
  
  // Find correct answer line
  const correctLineIndex = lines.findIndex((l) => 
    l.toLowerCase().includes("correct") || l.toLowerCase().includes("answer")
  );
  
  if (correctLineIndex === -1) {
    console.error("Quiz parse error: No correct answer found. Text:", text);
    return null;
  }
  
  const correctLine = lines[correctLineIndex];
  const correctMatch = correctLine.match(/(?:correct|answer)[:\s]+([A-D])/i);
  const correctAnswer = correctMatch ? correctMatch[1].toUpperCase() : null;
  
  if (!correctAnswer) {
    console.error("Quiz parse error: Could not parse correct answer. Line:", correctLine);
    return null;
  }
  
  const correctIndex = "ABCD".indexOf(correctAnswer);
  if (correctIndex === -1) {
    console.error("Quiz parse error: Invalid correct answer:", correctAnswer);
    return null;
  }
  
  // Find explanation (everything after correct answer or on the same line)
  const explanationStartIndex = correctLineIndex + 1;
  let explanation = "";
  if (explanationStartIndex < lines.length) {
    const explanationLines = lines.slice(explanationStartIndex);
    const explanationMatch = text.match(/(?:explanation|explain)[:\s]+(.*)/i);
    if (explanationMatch) {
      explanation = explanationMatch[1].trim();
    } else {
      explanation = explanationLines.join(" ").replace(/^(?:Explanation|Explain)[:\s]+/i, "").trim();
    }
  }
...
export const QuizViewer: React.FC = () => {
  ...
  const handleGenerateQuiz = async () => {
    ...
    try {
      ...
      setStatus("Generating with Chrome Built-in AI...");
      const prompt = QUIZ_PROMPT(inputText);
      const quizResult = await session.prompt(prompt);
      console.log("AI Response:", quizResult); // Debug log
      ...
      if (quizObj) {
        ...
        console.error("Failed to parse quiz. Raw response:", quizResult);
        setStatus("Failed to parse quiz. AI response format may be incorrect. Check console for details.");
        // Show raw response for debugging
        setQuiz({
          question: "Parsing Error - Raw AI Response:",
          options: [
            quizResult.substring(0, 100) + "...",
            "Check browser console for full response",
            "The AI response format may need adjustment",
            "Please try again with different content"
          ],
          correctIndex: 3,
          explanation: `The quiz generator returned text that couldn't be parsed. Raw response preview: ${quizResult.substring(0, 200)}...`
        });
      }
    } catch (error: any) {
    ...
    <div className="space-y-4 w-full max-w-full">
      <Card className="w-full">
        ...
        <CardContent className="p-4">
          <Textarea
            className="w-full border border-slate-200/60 rounded-xl p-3 min-h-[100px] mb-2 focus:ring-2 focus:ring-primary/20 transition-all"
